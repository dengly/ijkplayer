...
FF_ACT_ARCHS_ARMS="armv5 armv7a arm64"
...
case "$FF_TARGET" in
    ...
    ;;
    arms)
        echo_archs $FF_ACT_ARCHS_ARMS
        for ARCH in $FF_ACT_ARCHS_ARMS
        do
            sh tools/do-compile-libsoxr.sh $ARCH $FF_TARGET_EXTRA
        done
        echo_nextstep_help
    ;;
    clean)
